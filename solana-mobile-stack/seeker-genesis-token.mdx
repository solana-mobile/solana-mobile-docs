...
async function checkWalletForSGT(walletAddress) {
  ...
  try {
    ...
    const mintPubkeys = allTokenAccounts
      .map((accountInfo) => {
        try {
          ...
          if (info?.mint && info.tokenAmount?.amount !== "0") {
    ...
    for (let i = 0; i < mintPubkeys.length; i += BATCH_SIZE) {
      ...
      console.log(
        `Fetching mint info batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(mintPubkeys.length / BATCH_SIZE)}`,
      );
    ...
    console.log(
      `Checking ${mintAccountInfos.length} mints for SGT verification...`,
    );
      ...
      if (mintInfo) {
        ...
        try {
          ...
          if (
            ...
            const sgtMintAddress = mint.address.toBase58();
            console.log(
              `\nVERIFIED SGT FOUND: Wallet holds a verified SGT (${sgtMintAddress}).`,
            );
            return sgtMintAddress;
          }
        } catch (mintError) {
          // Skip this mint if we can't unpack it
          console.log(
            `Warning: Could not unpack mint ${mintPubkey.toBase58()}: ${mintError.message}`,
          );
          continue;
        }
      }
