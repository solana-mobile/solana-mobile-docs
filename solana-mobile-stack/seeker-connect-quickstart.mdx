...
Seeker Connect carries its wallet sessions over a Nostr relay. Set `relayDomain` to the relay domain below — accept the terms to reveal it, then drop it into the `registerSeekerConnect` call in the next step.
...
registerSeekerConnect({
  ...
  relayDomain: "<relay-domain>", // the Relay URL revealed above
});
```
