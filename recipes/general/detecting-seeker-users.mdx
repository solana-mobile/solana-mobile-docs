...
description: "Identify Seeker users in your app, from a lightweight device check to on-chain ownership verification."
---
...
Learn more about the [Seeker Genesis Token](/solana-mobile-stack/seeker-genesis-token).
...
<Note>
  This flow only authorizes and signs, so it uses no Solana library primitives.
  Apps on `@solana/web3.js` can import `transact` from
  `@solana-mobile/mobile-wallet-adapter-protocol-web3js` instead — the rest of
...
- Reference this [**example script**](/solana-mobile-stack/seeker-genesis-token#verifying-seeker-genesis-token-ownership) that uses the Helius `getTokenAccountsByOwnerV2` API. It returns the SGT's mint address if the wallet holds one, or `null` otherwise.
...
For full details and best practices, see the [Transferability](/solana-mobile-stack/seeker-genesis-token#transferability) documentation.
