...
In production, generate the `sign_in_payload` on your backend with a single-use `nonce`, `issuedAt`, and `expirationTime`, then verify the result server-side — including that the payload's `domain` is yours and the nonce hasn't been used before. See [Detecting Seeker Users](/recipes/general/detecting-seeker-users#step-1---prove-wallet-ownership-with-siws) for a complete example.
