---
title: "Mobile NFT Minter Tutorial"
description: "In this tutorial, you will learn how to build a React Native dApp that can mint NFTs from your phone photos."
sidebarTitle: "Build an NFT Minter"
---
...
<CardGroup columns={3}>
  <Frame>
    <img src="/images/static/tutorial_imgs/nftminter1.png" alt="Saga Photo 3" />
  </Frame>
  <Frame>
    <img src="/images/static/tutorial_imgs/nftminter2.gif" alt="Saga Photo 3" />
  </Frame>
  <Frame>
    <img src="/images/static/tutorial_imgs/nftminter3.png" alt="Saga Photo 3" />
  </Frame>
</CardGroup>
...
<Button href="https://github.com/solana-mobile/tutorial-apps/tree/main/MobileNFTMinter">
  View on Github
</Button>
...
- Complete [prerequisite setup](/get-started/development-setup) for a ready dev environment.
...
<Accordion title="A brief overview of each dependency:">
...
<Steps>
  <Step>Select a photo and upload it to a storage provider.</Step>
  <Step>
    Upload a JSON object containing metadata that conforms to the [Metaplex NFT
    Standard](https://www.metaplex.com/docs/smart-contracts/token-metadata/token-standard),
...
<Steps>
  <Step>
    Use the `rn-fetch-blob` library to read the image file into a Base 64
    string.
  </Step>
  <Step>Convert to raw bytes by decoding the Base64 string with `Buffer`.</Step>
  <Step>
    Use `fetch` to send a request containing the image bytes to the upload
    endpoint.
  </Step>
</Steps>
...
Next, we need to construct a metadata object that conforms to the [Metaplex NFT Standard](https://www.metaplex.com/docs/smart-contracts/token-metadata/token-standard), then
...
[`useMetaplex`](https://github.com/solana-mobile/tutorial-apps/blob/main/MobileNFTMinter/metaplex-util/useMetaplex.tsx): A React hook that vends a `Metaplex` instance with the `mobileWalletAdapterIdentity` installed.
...
With the `metaplex` instance, we can now access the `nfts()` module that provides [a collection of functions](https://github.com/metaplex-foundation/js) that
make it simple to interact with on chain programs and submit transactions.

To mint an NFT, call the `create` function which takes in a JSON object corresponding to the [Token Metadata Standard](https://www.metaplex.com/docs/smart-contracts/token-metadata/token-standard).
