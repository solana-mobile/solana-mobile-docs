...
Tuning is opt-in: `emulator start` and `emulator create --start` only apply the tweaks when you pass `--tune`, which is non-interactive and applies all of them. Running `tune` on its own asks which tweaks to apply, with every one pre-selected so `Enter` applies the lot; `-y` skips the picker.

Physical device serials are refused here — use [`device tune`](/cli/device#device-tune) for those.
