...
`--url` overrides the RPC endpoint while the wallet still sees the chain of the selected cluster. Mainnet always needs one, because the playground page runs in the device browser and Solana's public `api.mainnet-beta.solana.com` endpoint answers browser-origin requests with `403 Access forbidden`:
